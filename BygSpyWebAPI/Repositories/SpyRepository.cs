...
namespace BygSpyWebAPI.Repositories
{
    public class SpyRepository : ISpyRepository
    {
        ...
        public async Task<Spy?> GetSpyByEmailAsync(string email) =>
            await _spyCollection.Find(x => x.Creator_Email == email).FirstOrDefaultAsync();
